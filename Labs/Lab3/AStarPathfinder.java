 ...
 * Алгоритм реализован как статический метод, поскольку алгоритму поиска пути 
...
public class AStarPathfinder
{
     ...
     * указанной карты. Если путь может быть найден, возвращается путевая точка последнего шага пути; 
     * эта путевая точка может быть использована для обратного перехода к начальной точке. 
     * Если путь не может быть найден, возвращается <код>null</code>.
     **/
    public static Waypoint computePath(Map2D map)
    {
        ...
        while (!foundPath && state.numOpenWaypoints() > 0)
        {
            ...
            // Добавьте / обновите всех соседей текущего наилучшего местоположения. 
     ...
     * Новые путевые точки добавляются в коллекцию "открытые путевые точки" переданного объекта A* state.
     **/
    private static void takeNextStep(Waypoint currWP, AStarState state)
    {
        ...
        for (int y = loc.yCoord - 1; y <= loc.yCoord + 1; y++)
        {
            for (int x = loc.xCoord - 1; x <= loc.xCoord + 1; x++)
            {
                ...
                // Хорошо, мы обманываем и используем смету затрат для вычисления фактической
                ...
                //для этого местоположения уже существует путевая точка, новая
                //путевая точка заменяет старую путевую точку только в том случае, если она дешевле старой.
     ...
     * Вычисленная фактическая стоимость - это просто расстояние по прямой между двумя точками.
     **/
